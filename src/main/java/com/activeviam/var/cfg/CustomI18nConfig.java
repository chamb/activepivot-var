...
public class CustomI18nConfig {
	...
	public static void pushTranslations(IActivePivotContentService apContentService) {
		final IContentService rootContentService = apContentService.getContentService()
				.withRootPrivileges();
		if (!rootContentService.exists(I18N_FOLDER)) {
			rootContentService.createDirectory(I18N_FOLDER,
					Collections.singletonList(ROLE_ROOT),
					Collections.singletonList(ROLE_ROOT),
					false);
		}
	...
	protected static void push(PrefixedContentService contentService, String languageFolder)
		...
		for (Resource resource : resources) {
			...
			contentService.createFile(name,
	...
	public Void initializeCubeFormatters() {
		...
		final IPlugin<ICubeFormatterFactory> factories = Registry
				.getPlugin(ICubeFormatterFactory.class);
