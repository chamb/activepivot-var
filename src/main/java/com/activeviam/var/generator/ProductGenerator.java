...
public class ProductGenerator {
	...
	protected static final String[] underlierCodesSingleStock = {
			".EPEU", ".GDAXIP", "1mEUEB.RBS", "AAH.AS", "AGFP.PA", "ALBK.I", "BASF.DE", "BKIR.I",
			"BNPP.PA", "COR.AS",
			"CRDI.MI", "DTEGn.DE", "ELE.MC", "ENI.MI", "EONG.DE", "ENI.MI", "EONG.DE", "FTE.PA", "HICP",
			"INWS.I", "IPM.I", "ISPA.AS", "KESBV.HE", "LOIM.PA", "LYOE.PA", "MANG.DE", "MMTP.PA",
			"RWEG_p.DE",
	 ...
	 *
	 * @param productId the id of the product to generate
	 * @param random random generator to use (this allows to provide a seeded random for repeatable
	 * 		scenarios).
	 * @return product
	 */
	public Product generate(int productId, Random random) {
		...
		if (product.getUnderlierType().equals("EquityIndex")) {
			product.setUnderlierCode(underlierCodesEquityIndex[random
					.nextInt(underlierCodesEquityIndex.length)]);
		} else {
			product.setUnderlierCode(underlierCodesSingleStock[random
					.nextInt(underlierCodesSingleStock.length)]);
		}
		//productType
		...
		//baseMTM
		product
				.setProductBaseMtm(RiskCalculator.round(RiskCalculator.nextDouble(500, 1000, random), 2));
		// Theta
